import { __SYD, __c, __m, __sC , __p} from './sydneyLib/sydneyDom.js'

__SYD.board_pole = () =>{
    return __c(
        'div',
        {//calc(400px - 70px)
            style:`min-height:${__p(['container','scale'],false) ? '1200px' : '900px'};width:${__p(['container','scale'],false) ? '150%' : '120%'};min-width:600px;max-width:${__p(['container','scale'],false) ? '1500px' : '1300px'};position:absolute;bottom:-50px;left:-800px;transform:translateZ(-2400px) translate(-35% , ${__p(['container','scale'],false) ? '450px' : '-10px'}) rotateY(60deg) ;transform-style:preserve-3d;perspective-origin:50% 50%;background:unset;z-index:390;`,
            class:"pole"
        },
        [
            __SYD.pole_front(),
